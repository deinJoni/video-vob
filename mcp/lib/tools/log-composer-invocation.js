...
function logComposerInvocation(args) {
  ...
  return withSessionLock(id, () => {
    ...
    const composition = state.composition && typeof state.composition === "object" && !Array.isArray(state.composition)
      ? state.composition
      : null;
    const revisionCountAtInvocation = composition && Number.isInteger(composition.revision_count)
      ? composition.revision_count
      : 0;

    const ts = nowIso();
    const event = {
      kind: "composer_invoked",
      at: ts,
...
module.exports = Object.freeze({
  name: "vob_log_composer_invocation",
  description: "Append a `composer_invoked` audit event to state.history before the orchestrator spawns the composer subagent. Records the revision_count at invocation time and optional revision_notes (used on re-invocation passes — either user-driven revision or lint-driven auto-retry). Does NOT mutate state.composition — the subagent's own vob_save_composition call does that.",
  inputSchema: {
    type: "object",
    properties: {
      ...
      revision_notes: {
        ...
        description: "Optional. The user-facing revision request or lint findings summary that triggered this invocation. Omit on the first invocation of a project.",
      },
    },
    required: ["project_id"],
  },
  handler: logComposerInvocation,
