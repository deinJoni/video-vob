...
2. `mcp__vob__vob_snapshot_keyframes { project_id, timecodes }` (works in COMPOSE — needs only a
...
2b. `mcp__vob__vob_qc_stills { project_id, timecodes }` — pass the SAME `timecodes`. Automated
...
3. **Spawn the `visual-critic`** — the independent pixel critic — UNLESS the summary's
   ...
   ```
   Task(subagent_type: "visual-critic",
        description: "Visual critique of rendered stills",
        prompt: "DATA
   project_id: <project_id>
...
4. **Act on the verdict — at most ONE critic-driven fix** (mirrors PLAN step 6c):
   ...
   - **`REVISE`** and not already auto-fixed this COMPOSE pass → `mcp__vob__vob_log_composer_invocation
...
7. **Present, then branch.** Present to the user: file list, total duration, the contact sheet
   ...
   prominently. Then:
   - **Approve** → `mcp__vob__vob_transition_phase { project_id, to_phase: "PREVIEW" }`. (There is
   ...
   - **Back to the plan** → `mcp__vob__vob_transition_phase { project_id, to_phase: "PLAN" }` and
     re-enter PLAN.
