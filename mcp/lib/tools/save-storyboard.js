...
module.exports = Object.freeze({
  ...
  description: "Save the storyboard (content may be a JSON object or string). Schema 1.0 = one timeline (scenes[] + total_target_duration_seconds); 1.1 adds the multi-short fan-out form (top-level shorts[], scene_ids globally unique); 1.2 adds typed overlay objects in scene.overlays[], narrative segments[] (+ render_segmentation single|auto|manual), target.fps, and richer broll_placements (render_mode full_frame|pip|overlay|subject — subject mattes a clip off its background via hyperframes remove-background and composites it over a backdrop {kind: design_token|clip_ref|scene_base; NO synthesized/stock backdrops}, optional position{anchor,scale} + motion{in,out}; plus the rectangular modes' motion note, and the GAP form: {source:'gap', description, desired_duration_seconds, scene_ref} when the ingested footage lacks coverage — gaps collect into plan/broll_gaps.json and warn PLAN_BROLL_GAP_UNFILLED, resolved by ingesting more footage via the PLAN→INGEST back-edge). Additive, non-version-gated (any schema version): optional target.design (structured Design-language tokens — palette/typography/caption_style/motion/grade), richer per-scene caption_segments (emphasis_words[], animation pop|word-by-word|karaoke, style_ref, position), and optional per-clip source_clips[].speed (0.25–4.0, default 1; constant playback rate baked in at materialize via setpts/atempo — on-screen length = (out-in)/speed, so author target_duration_seconds in OUTPUT time). Validates shape, then runs plan lint (ruleset from the active video-type preset; per short in fan-out): errors (out-of-range clips, captions-on-silent, narration-span violations, duplicate scene_ids, overlay out-of-bounds) reject the save; warnings (hook placement/length under retention, pacing monotone / inverted rhythm arc, duration drift or per-short range, b_roll holds, key-moment coverage, clean-speech straddles when clean_cut, overlay dwell/conflict/safe-area, video budget incl. PiPs, chapters-missing, caption chunk-too-long / emphasis-not-in-text / timing-drift / karaoke-on-unaligned-transcript, subject backdrop-not-ingested / budget-exceeded) return in plan_lint, persist to state, and render into storyboard.md for the plan gate. Any save resets confirmed:false and bumps revision_count.",
  inputSchema: {
    type: "object",
    properties: {
