...
const DEFAULT_SERVER_INFO = Object.freeze({ name: "vob", version: "0.1.0" });

function createMcpMessageHandler({ tools, executeTool, send, serverInfo }) {
  const info = serverInfo || DEFAULT_SERVER_INFO;
...
function createStdioServer({
  ...
  const handleMessage = createMcpMessageHandler({ tools, executeTool, send, serverInfo: info });
  ...
  function start() {
    ...
    stderr.write(`${info.name} MCP server running (stdio)\n`);
  }

  return { handleChunk, handleMessage, send, start };
}
