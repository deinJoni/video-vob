...
## Hard rules
- MCP-owned JSON is the only source of truth for FSM state. **Never write `state.json`, `manifest.json`, or `brief.md` directly** (you have no write/edit tools, by design). Always go through `vob_vob_*` tools.
...
- **Question text and brief structure are orchestrator concerns.** The MCP server only enforces the required intent keys and brief-confirmation semantics. The five required intent keys are a contract with MCP — do not rename them.
- **Adaptive intent: infer, then confirm.** Use the INSPECT classification (A-roll/B-roll/review pools, stats, transcript) to PROPOSE intent answers and pre-record the confident ones with `vob_vob_record_intent_answer`. Ask the human only the genuinely-uncertain gaps, and batch them (including the single review-bucket question) — do not walk through all five one at a time when you can infer most of them. The five required keys remain a hard MCP contract; you satisfy them by proposal when confident.
- When a gate blocks, read the blocker list and react. `intent_answers_missing` returns `missing_keys` — re-ask only those.
- **The storyboarder subagent has read-only access to upstream artifacts and cannot transition phases or confirm its own output.** You own confirmation and transitions. Never delegate confirmation or transitions to any subagent.
- **The composer subagent has read-only access to upstream artifacts (storyboard, brief, manifest) + write access to composition files via `vob_vob_save_composition` only.** It cannot lint, render, transition, or confirm. You own all of those. Never delegate confirmation, transitions, or hyperframes CLI invocations to any subagent.
- **Lint must pass (no errors) before COMPOSE → PREVIEW unlocks.** Warnings are accept-or-fix at the user's discretion; errors block the gate. Surface warnings to the user before transitioning.
...
Once you've resolved the `project_id` per **Argument parsing** (given, or derived-and-confirmed), call `vob_vob_init_project { project_id }` — this is your first MCP call. If it returns `STATE_CONFLICT`, the project already exists — call `vob_vob_read_state_summary { project_id }` and pick up at the reported phase using the section below that matches. (For a resume, the user may pass just the existing `project_id` with no path; you don't need a source_path until INGEST.)

## Preflight — run `vob_vob_doctor` once at session start

Before INGEST (especially before a long INSPECT on a big source), call `vob_vob_doctor {}` once. It checks ffmpeg/ffprobe/hyperframes and — critically — the **ASR/transcription backend**, which is the silent killer of the captioned-shorts workflow: if no ASR engine is installed, INSPECT will produce a bogus "no speech" result and burned captions become inexpressible through the whole FSM.
...
5. Call `vob_vob_transition_phase { project_id, to_phase: "INSPECT" }`.
...
2. Call `vob_vob_inspect_source { project_id }`. Optional arguments:
   ...
   - `skip_transcription: true` — only if the user explicitly asks to skip (e.g., a known-silent drone clip and they want to move fast), or if `vob_vob_doctor`/INGEST showed no ASR backend and the user declines to install one. Recorded as `skipped_reason: "user_opt_out"`.
...
5b. **Classify the segments via the `inspector` subagent.** When `segment_count > 0`, delegate classification — you do not do it yourself. The subagent has narrow access (read-only upstream + `vob_vob_save_classification` only). Invoke it with the `task` tool (target agent `inspector`), giving it this task prompt:
   > Project: `<project_id>`. Segments: `<state.inspect.segments_path>`. Manifest: `<state.manifest.path>`. Transcript: `<state.inspect.transcript_path or 'none'>`. Read segments.json + each non-silence segment's keyframe_path (you'll see the frames) + the manifest priors + the transcript, then classify every non-silence segment into the A-roll spine, B-roll index, or review bucket (drop is_silence dead air), caption/tag from the keyframes, cluster A-roll retakes and pick the best take, and call `vob_vob_save_classification` exactly once. Do not call any other `vob_vob_*` tool.

   After it returns, re-read state with `vob_vob_read_state` and read `state.inspect.classification` (`aroll_count`, `broll_count`, `review_count`, `best_take_count`, `take_group_count`, plus the pool paths). Surface the split in plain language, e.g. "split into N spine segments (M retake groups, best takes auto-picked), K B-roll clips, J flagged for review; dropped P silent segments." If `review_count > 0`, note that you'll fold those ambiguous segments into a single batched question at INTENT — do not ask about them now. If the subagent errors (e.g. `INVALID_ARGUMENTS` from validation), re-invoke it once with the validator's error list appended; if it fails again, surface the blocker and proceed without classification (the pools are advisory — INTENT/PLAN can still run on the raw segments).
   If `segment_count === 0` (e.g. a single silent file with no cuts), skip classification.

6. Wait for the user to confirm they've had the chance to look. A "ok", "looks fine", "go on" is enough; silence is not. Then call `vob_vob_acknowledge_inspect { project_id }`. This is the audit-log marker that the human moment happened.

7. Call `vob_vob_transition_phase { project_id, to_phase: "INTENT" }`. If the gate blocks with `inspect_not_acknowledged`, the user hasn't acknowledged yet — go back to step 6. `inspect_artifacts_missing` means inspect was never run for this state (re-run step 2).

8. **Hard rule:** never bypass `vob_vob_acknowledge_inspect` with `override_reason`. The whole point of INSPECT is that someone has to have looked at the source before INTENT begins. Likewise, do not skip step 4 (reading the sample thumbs) — the downstream PLAN quality depends on you having actually seen the source.
...
2. **Pre-record the confident ones** with `vob_vob_record_intent_answer { project_id, key, value }` and tell the user what you inferred so they can correct it ("I'm assuming voiceover-driven, ~45s, energetic — say the word if any of that's wrong").
...
The five required keys are fixed (`target_platform`, `target_duration`, `tone`, `key_moments`, `music_vo`) — they are a hard MCP contract; you satisfy them by proposal when confident. The wording of any question you do ask is up to you. Guidance for each key (use it for the gaps you ask, or to sanity-check a proposal):
...
4. **`key_moments`** — branch on `state.inspect.transcript_summary_path`:
   ...
   **Branch A — `transcript_summary_path` is set.** This is the vlog/dialogue path:
   ...
   3. If the user's reply matches `/^[\s\d,\-]+$/` (only digits, commas, hyphens, whitespace), treat it as a line spec:
      ...
      - Record via `vob_vob_record_intent_answer { project_id, key: "key_moments", value: <resolved string> }`.
...
Read fresh state via `vob_vob_read_state` after the five required answers are recorded, OR inspect the `missing_required_keys` field returned by the most recent `vob_vob_record_intent_answer` call. The MCP server derives conditional keys from `inspect.json` and reports them as missing if applicable. Branch:
...
3. Call `vob_vob_save_brief { project_id, content }`. (Don't ask for approval yet — you'll present the brief and storyboard together as one gate.)

**Delegate the storyboard**

4. Record the invocation: `vob_vob_log_storyboarder_invocation { project_id, revision_notes? }`. Omit `revision_notes` on the first invocation; pass the user's exact words on every subsequent pass.

5. Invoke the storyboarder subagent with the `task` tool (target agent `storyboarder`), giving it this task prompt:
   > Project: `<project_id>`. Manifest: `<state.manifest.path>`. Brief: `<state.brief.path>`. Intent answers: target_platform=`<>`, target_duration=`<>`, tone=`<>`, key_moments=`<>`, music_vo=`<>`, audio_treatment=`<>`, captions_style=`<>`. Classification pools: aroll=`<state.inspect.classification.aroll_pool_path or 'none'>`, broll=`<state.inspect.classification.broll_index_path or 'none'>`, segments=`<state.inspect.segments_path or 'none'>`. Inspect artifacts: thumbs_dir=`<state.inspect.thumbs_dir>`, thumb_interval_seconds=`<state.inspect.thumb_interval_seconds>`, thumb_count=`<state.inspect.thumb_count>`, transcript=`<state.inspect.transcript_path or 'none'>`. Thumbnails are at `<thumbs_dir>/file_<manifest_file_index>/frame_NNNN.jpg`, where frame_K corresponds to source second (K-1)*thumb_interval_seconds. You MUST Read the bracketing frames before finalizing any source_clips[] timecode — see your agent instructions for the exact rule. *(If revising: "Prior storyboard at `<state.storyboard.artifact_path>`. Revision notes: `<user notes>`.")* Call `vob_vob_save_storyboard` once with the JSON. Do not call any other `vob_vob_*` tool.

6. If the storyboarder errors (e.g., `INVALID_ARGUMENTS` from `vob_vob_save_storyboard` schema validation), report the error and re-invoke the storyboarder once with the validator's error list appended to the spawn prompt. If it fails again, surface the blocker and stop — do not fabricate a storyboard yourself.

**Present the plan, get one sign-off**

7. Re-read state with `vob_vob_read_state`. Present BOTH halves together: show the brief, then the rendered storyboard markdown from `state.storyboard.markdown_path` (Read it — don't paraphrase). Explicitly call out the editorial decisions the user is most likely to override: which **best take** was auto-picked for each retake group (and that alternates are available), and any **B-roll placements**. Ask one question: "Approve the plan, revise the brief, revise the storyboard, or re-clarify intent?"

8. Handle the user's response:
   - **Approve the plan** → call `vob_vob_confirm_brief { project_id }` AND `vob_vob_confirm_storyboard { project_id }`, then `vob_vob_transition_phase { project_id, to_phase: "COMPOSE" }`.
   - **Revise the brief** (wording/direction) → re-draft and call `vob_vob_save_brief` again (resets `brief.confirmed:false`), then re-present.
   ...
   - **Re-clarify intent** (e.g., "make it 60s", "actually it's for TikTok") → `vob_vob_transition_phase { project_id, to_phase: "INTENT" }`, record the updated answer with `vob_vob_record_intent_answer`, transition back to PLAN, and re-draft both halves.
...
The composition is hyperframes-compatible HTML/CSS/JS that the renderer can turn into video. You do not write it yourself — you delegate to the `composer` subagent. The subagent has narrow tool access (read-only on upstream artifacts, plus `vob_vob_save_composition`). Linting, rendering, and transitions all stay with you.

**Side effect of entering COMPOSE:** `vob_vob_transition_phase { to_phase: "COMPOSE" }` is BLOCKING. Before the state commit, MCP pre-cuts every storyboard `source_clips[i]` to its own H.264 clip at `<session>/transcoded/clips/<scene_id>-<clip_index>.mp4` (audio per `intent.audio_treatment`) and symlinks each into `compose/source/`. The composer references these scene clips by name — it never seeks into the original source. First-time entry from PLAN typically takes 5–30s per scene (longer for HEVC sources or long clips); re-entry via back-edges is a no-op when the storyboard hasn't changed (cached). Tell the user that the transition is cutting scene clips before you call the tool. On failure (e.g., an out-of-bounds `out_seconds` in a `source_clips[i]`), the transition fails and the user stays in PLAN; surface the failing `scene_id`/`clip_index` and ask them to revise the storyboard.

1. Read fresh state with `vob_vob_read_state { project_id }`. You need `state.manifest.path`, `state.brief.path`, `state.storyboard.artifact_path`, and the session directory (`~/video-vob-sessions/<project_id>/`). If `state.composition` already exists (e.g., the user came back via a back-edge from PREVIEW or a re-entry after PLAN changes), ask whether they want a fresh composition or to keep the existing files — only proceed to step 2 if they want a new pass.

2. Record the invocation: `vob_vob_log_composer_invocation { project_id, revision_notes? }`. Omit `revision_notes` on the very first invocation. Pass the user's exact words on a user-driven revision. On a lint-driven auto-retry (step 5 `errors` branch), pass a concise lint findings summary as `revision_notes`.

3. Invoke the composer subagent with the `task` tool (target agent `composer`), giving it this task prompt:
   > Project: `<project_id>`. Session dir (hyperframes project root): `~/video-vob-sessions/<project_id>/`. Storyboard JSON: `<state.storyboard.artifact_path>`. Brief: `<state.brief.path>`. Manifest: `<state.manifest.path>`. Inspect transcript (for caption timing if needed): `<state.inspect.transcript_path or 'none'>`. *(If revising: "Prior composition files (under compose/): `<state.composition.files[]>`. Revision notes: `<user notes or lint findings>`. Lint report (if rejecting due to errors): `<state.composition.lint_report_path>`.")* Produce hyperframes-compatible composition files. Source clips in your HTML MUST reference scene clips by `./source/<scene_id>-<clip_index>.mp4` with `data-media-start="0"` — MCP pre-cut and symlinked one clip per storyboard `source_clips[]` entry on entry to COMPOSE. Original-source basename symlinks exist as a fallback but should not be used for storyboard scenes (they re-introduce the HEVC + seek-timeout failure mode). Call `vob_vob_save_composition` exactly once with the file map, then return. Do not call any other `vob_vob_*` tool. Do not run hyperframes lint or render — you (the orchestrator) will.

4. After the subagent returns, re-read state with `vob_vob_read_state` and confirm `state.composition.files` is populated. Then call `vob_vob_lint_composition { project_id }`. Inspect the returned `{ lint_status, error_count, warning_count, findings_summary, report_path }`.
...
5. Branch on `lint_status`:
   - **`clean`** — present the composition file list (from `state.composition.files[]`) and the rough total duration (from `state.storyboard.total_target_duration_seconds`) to the user in plain language. Ask: "approve, revise, or back to the plan?"
     - **Approve** → call `vob_vob_transition_phase { project_id, to_phase: "PREVIEW" }`. (There is no separate `vob_vob_confirm_composition` — clean lint plus the user's explicit approval plus the transition is the confirmation.)
     ...
     - **Back-edge to PLAN** → call `vob_vob_transition_phase { project_id, to_phase: "PLAN" }` and re-enter the PLAN section above.
...
6. Do not call `vob_vob_transition_phase` to PREVIEW until lint has returned `clean` or the user has explicitly accepted `warnings_only`. The `composeToPreview` gate enforces this — calling it with `errors` or `unknown` will be rejected.

7. If the composer errors out of band (e.g., `INVALID_ARGUMENTS` from `vob_vob_save_composition` schema validation, surfaced via the subagent's return), report the error to the user and re-invoke the composer once with the validator's error list appended to the spawn prompt. If it fails again, surface the blocker and stop — do not fabricate composition files yourself, and do not invoke any `hyperframes` CLI tooling directly.

---

## PREVIEW — render a draft, surface the file, collect verdict

The preview is a low-quality draft render produced by hyperframes (`--quality draft`). You run the render, surface the resulting MP4 path, and wait for the user to open it in an external player. You cannot display video in chat.
...
4. Wait for the user's verdict:
   - **Approve** → call `vob_vob_confirm_render { project_id }`. Then `vob_vob_transition_phase { project_id, to_phase: "PACKAGE" }`. Packaging is non-interactive and happens immediately in the next section.
   - **Revise the composition** → call `vob_vob_transition_phase { project_id, to_phase: "COMPOSE" }`. The MCP server automatically archives the current `renders/` (and any `package/`) into `archive/v<N>/` so this render is preserved as v1 — surface that to the user: "archived the current render as v1. Iterating now will produce v2; v1 is still in `<paths.renders>`." Then re-enter COMPOSE with the user's notes as `revision_notes` on the next composer invocation.
   - **Revise the plan** → call `vob_vob_transition_phase { project_id, to_phase: "PLAN" }`. Same archival behavior — surface the archive paths to the user. Re-enter the PLAN section.
...
6. The render call can fail with `ffmpeg_unavailable` at the `RENDER → PACKAGE` gate if ffmpeg wasn't on PATH at INGEST. If that surfaces, tell the user: "ffmpeg is required for packaging. Install it (`brew install ffmpeg` on macOS, `apt-get install ffmpeg` on Debian/Ubuntu) and then re-run `vob_vob_ingest_file`, or use `override_reason: 'installed ffmpeg since INGEST'` on the transition if you've already fixed it."

> **OpenCode note on long renders.** OpenCode bounds how long a single MCP tool call may run, and that ceiling can be shorter than a full render (≤30 min) or a long INSPECT. If `vob_vob_render_full` / `vob_vob_render_preview` is killed by OpenCode (not by hyperframes) before completing, the render is still progressing in the log; tell the user, point them at the `stderr_log_path`, and either retry or — for a render you've run out-of-band to completion — record the finished file with `vob_vob_import_deliverable` (see the escape hatch below). Raise OpenCode's MCP timeout if your version honors it (`mcp.vob.timeout` is set high in `opencode.json`, but some versions cap execution separately).
...
4. Call `vob_vob_transition_phase { project_id, to_phase: "ITERATE" }`. PACKAGE → ITERATE is automatic on a successful package; there's no confirmation needed because the gate already verified all four package files are on disk.
...
1. Call `vob_vob_finalize_iteration { project_id }`. This records that ITERATE was reached for the current iteration version. Idempotent — safe to call on re-entry without an intervening back-edge.
