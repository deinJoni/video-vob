...
- **Scratch space:** bespoke ffmpeg/hyperframes work (cut/concat/speed-ramp bases, transparent
  ...
  write-guard allows exactly that subtree (spine rule 8). Get the user's go-ahead before leaving
  the rails, and record every resulting final immediately below.
- **How to invoke hyperframes (NOT npx).** Run `mcp__vob__vob_doctor` first: its `hyperframes`
...
- **QC the off-rails composition before you render it.** Off-rails work used to skip lint/QC
  ...
  `mcp__vob__vob_lint_composition { project_id, compose_dir: "<…>/work/<your-comp-dir>" }` on the
