...
3b. **Surface full-resolution key-frame stills for the USER.** A draft render hides text
   legibility and fps-dependent motion. Call `mcp__vob__vob_snapshot_keyframes { project_id,
...
4. Wait for the user's verdict:
   - **Approve** → `mcp__vob__vob_confirm_preview { project_id }`, then
     `mcp__vob__vob_transition_phase { project_id, to_phase: "RENDER" }`.
