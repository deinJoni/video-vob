...
- ✅ Done & committed: planning (57a22e7), INSPECT (06d879b), PLAN P1 (bd47d88) + P2/P3 (7577c51), COMPOSE (b01e2d8), PREVIEW+RENDER shared XC-1/XC-2 (6c18084).
- **NEXT ACTION ON WAKE: Stage 7 RENDER P2** (the shared XC-1/XC-2 are already done). In order:
  1. **Segment-confirm at assembly** (RENDER#1): `render-segments.js::validSegmentRenders` must require `entry.confirmed===true`; `assemble-video.js` names unconfirmed in its STATE_CONFLICT. Small.
  2. **Music duck surfaced/tunable** (RENDER#3): `assemble.js` — surface `ducked` into `assemble_video` result + manifest/README; warn on flat-mix fallback; optional tunable duck params.
  3. **Screenshot-path fallback on `<video>`-budget render timeout** (RENDER#2): `render-full.js` — on timeout when video count > `hostProfile.videoBudget()`, retry once with `PRODUCER_FORCE_SCREENSHOT=1` before throwing.
- Then **PACKAGE** (loudnorm per video-type — BIGGEST lever: `loudness_target{i,tp,lra}` per preset in `video-types.js`, thread `resolveActiveVideoType` into `loudnorm.js`; + LRA-aware skip; + word-level VTT; + INSPECT#4a measured-LUFS reuse), **ITERATE** (compare_iterations quality deltas from archived manifest; archival sweeps segment_renders; finalize completeness backstop), **CORE** (short_id on preview/render slots+guard, process-level lock release, intentToPlan canonical path, + deferred stale-vs-intent gate + PREVIEW#9 confirm-revision + brief-validator audio).
- Verification that works without a source: `node -e` targeted tests, `node scripts/m5-walker.js spans`, boot, and real ffmpeg-generated test clips for probe/loudnorm logic. Heavy walker phases need VOB_WALKER_SOURCE+hyperframes.
