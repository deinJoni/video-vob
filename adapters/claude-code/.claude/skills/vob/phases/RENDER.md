...
4b. **Fan-out record-then-loop** (the storyboard has `shorts[]`). After `mcp__vob__vob_confirm_render`:
   1. Record the finished short:
      `mcp__vob__vob_import_deliverable { project_id, deliverables: [{ path: <mp4_path>, title:
   ...
   3. **All shorts recorded** → `mcp__vob__vob_transition_phase { project_id, to_phase: "PACKAGE" }`. The gate blocks
