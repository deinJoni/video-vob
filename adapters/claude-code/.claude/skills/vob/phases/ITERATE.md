...
1. Call `mcp__vob__vob_finalize_iteration { project_id }`. Records that ITERATE was reached for
...
3. Handle the response:
   ...
   - **Revise composition** → `mcp__vob__vob_transition_phase { project_id, to_phase:
...
5. **"What changed between cuts?"** — once two or more iterations are archived, answer with
   `mcp__vob__vob_compare_iterations { project_id }` (read-only; defaults to the two most recent
