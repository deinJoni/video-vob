...
5b. **Classify the segments via the `inspector` subagent.** When `segment_count > 0`, delegate —
   ...
   ```
   After it returns, read `mcp__vob__vob_read_state_summary` → `inspect.classification` (NOT a
...
6. Wait for the user to confirm they've had the chance to look. A "ok", "looks fine", "go on" is
   enough; silence is not. Then call `mcp__vob__vob_acknowledge_inspect { project_id }`.

7. Call `mcp__vob__vob_transition_phase { project_id, to_phase: "INTENT" }`. If the gate blocks
