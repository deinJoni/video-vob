...
`vob_vob_record_intent_answer { project_id, key, value }` (record the user's words; the server
...
4. **`key_moments`** — branch on `transcript_summary_path`:
   ...
   **Branch A — `transcript_summary_path` is set** (the vlog/dialogue path):
   ...
   3. If the reply matches `/^[\s\d,\-]+$/` (only digits, commas, hyphens, whitespace), treat it
      ...
      - Record via `vob_vob_record_intent_answer { project_id, key: "key_moments",
...
After all applicable keys are recorded, call `vob_vob_transition_phase { project_id,
